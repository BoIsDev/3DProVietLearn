...
namespace VoxelBusters.EssentialKit
{
    ...
    public static class AddressBook
    {
        ...
        private     static  INativeAddressBookInterface    NativeInterface
        {
            get
            {
                if(s_nativeInterface == null)
                {
                    DebugLogger.LogException(EssentialKitDomain.Default, new Exception("Address book is not enabled. You need to enable this feature to use it."));
                }
        ...
        public static void Initialize(AddressBookUnitySettings settings)
        {
            ...
            // Configure interface
            NativeInterface       = NativeFeatureActivator.CreateInterface<INativeAddressBookInterface>(ImplementationSchema.AddressBook, true);
        }
        ...
        public static AddressBookContactsAccessStatus GetContactsAccessStatus()
        {
            ...
            {
                // make request
                return NativeInterface.GetContactsAccessStatus();
            }
        ...
        public static void ReadContacts(EventCallback<AddressBookReadContactsResult> callback)
        {
            ...
            {
                // make request
                NativeInterface.ReadContacts((contacts, error) => SendReadContactsResult(callback, contacts, error));
            }
            catch (Exception exception)
            {
                DebugLogger.LogException(EssentialKitDomain.Default, exception);
        ...
        private static void RequestContactsAccessInternal(EventCallback<AddressBookRequestContactsAccessResult> callback)
        {
            ...
            {
                // make request
                NativeInterface.RequestContactsAccess((status, error) =>
                {
